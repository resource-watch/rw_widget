...
class WidgetsIndex
  ...
  delegate :widgets_url, to: :controller
  ...
  def widgets
    @widgets ||= Widget.fetch_all(options_filter)
                       .order(sort_params)
  ...
  private
    ...
    def options_filter
      params.permit(:status, :published, :verified, :app, :template, :name, :dataset, :default, :sort, :logged_user, :widget, logged_user: {}, widget: {}).tap do |filter_params|
        filter_params[:page]= {}
